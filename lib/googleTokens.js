...
class GoogleTokens {
  constructor ({ client_id: id, client_secret: secret, callback = 'http://localhost:3000/callback' }) {
    // create an oAuth client to authorize the API call
    this.oAuth2Client = new google.auth.OAuth2(id, secret, callback)
  }

  // Create http server to accept the oauth callback.
  async authPrompt ({ scope = [], prompt = '', access = 'offline' } = {}) {
